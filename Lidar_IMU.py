...
def imu_reader_thread(port=IMU_PORT, baud=IMU_BAUD, out_csv=IMU_CSV):
    try:
        ...
        return
    # ヘッドレス化：IMU はファイルに逐次書き出さず、共有バッファへ格納だけ行う（CSV は LIDAR 側で統合して出力）
    ...
    while not stop_event.is_set():
        try:
            ...
            with open(out_csv, 'a', newline='') as f:
                ...
                # 逐次測定ループ（バッファ制限を厳しく）
                # for meas in lidar.iter_measures(max_buf_meas=512):
                for scan in lidar.iter_scans(max_buf_meas=512):   # スキャン単位で安定化（max を下げる）
                    if stop_event.is_set():
                        break
                    # シリアル内部バッファが大きくなってきたらクリアしてリカバリ（安全策）
                    try:
                    ...
                    # writer.writerow([scan_t, x, y, r, quality, angle, imu_t, ax, ay, az, pitch, roll])
                    # 新: キューに投げる（full時は古い行を破棄して最新を入れる）
                    row = [scan_t, x, y, r, quality, angle, imu_t, ax, ay, az, pitch, roll]
                    try:
                        csv_queue.put_nowait(row)
                    except queue.Full:
                        try:
                            _ = csv_queue.get_nowait()   # 古いエントリを破棄
                            csv_queue.put_nowait(row)
                        except queue.Empty:
                            pass
            ...
            # rplidar の例外に対してはシリアルバッファをリセットして再試行
...
def get_latest_imu(t_sec):
    ...
    返却: (t,ax,ay,az,pitch,roll) または None。呼び出しは imu_lock で囲っても良い。"""
    with imu_lock:
        if not imu_buf:
            return None
        # imu_buf は時系列で append されている想定 -> リスト化して bsearch
        lst = list(imu_buf)
    # lst[i][0] が時刻（秒）である想定
    ...
    # 応用: ここで閾値（例 0.05s）を超えるなら None を返す選択も可能
    return cand
...
def euclidean_cluster(points, eps=0.25, min_pts=3):
    """単純な single-link クラスタ（スキャン内点数が少ない想定で高速化不要）"""
    N = points.shape[0]
    if N == 0:
        return []
...
def manage_tracks(detections, now, gate=0.6, max_misses=5, max_tracks=12):
    ...
    # 3) create new tracks for unassigned detections (limit total)
    for j in range(N):
...
def process_scan_and_tracks(now=None):
    """最新スキャンをクラスタ化→重心を検出→トラッキング更新。返り値は現在のトラック一覧（x,y,vx,vy,id）"""
    if now is None:
        now = time.time()
    ...
    try:
        while not stop_event.is_set():
            ...
            if now - last_print >= 1.0:
                ...
                s = f"[{time.strftime('%H:%M:%S')}] points={n_pts} imu={n_imu} csv_q={qsize}"
                if tracks_summary:
                    s += " | tracks: " + ", ".join([f"id{tid}:({x:.2f},{y:.2f}) v({vx:.2f},{vy:.2f})" for tid,x,y,vx,vy in tracks_summary])
                print(s)
                last_print = now
